...
Add the follwing dependency to your `pom.xml`.
----
<dependency>
...
Now, configure it with the parameters in the `application.properties` configuration file (at least set `camel.mdc.enabled=true`).
...
If you want to use the feature, you need to create and confige an instance of the `MDCService` and add it to your CamelContext via `service.init(context)`.
...
|`camel.mdc.customExchangeHeaders` |  | Provide the headers you would like to use in the logging. Use `*` value to include all available headers.
|`camel.mdc.customExchangeProperties` |  | Provide the properties you would like to use in the logging. Use `*` value to include all available properties.
|=======================================================================
